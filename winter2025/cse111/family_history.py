...
def print_death_age(people_dict):
    ...
    for person, info in people_dict.items():
        print(f'{info[NAME_INDEX]} {int(info[DEATH_YEAR_INDEX]) - int(info[BIRTH_YEAR_INDEX])}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    """
    print("Marriages")
    for marriage, info in marriages_dict.items():
        husand = info[HUSBAND_KEY_INDEX]
        wife = info[WIFE_KEY_INDEX]
        print(f'{people_dict[husand][NAME_INDEX]} {int(info[WEDDING_YEAR_INDEX]) - int(people_dict[husand][BIRTH_YEAR_INDEX])} > {info[WEDDING_YEAR_INDEX]} < {people_dict[wife][NAME_INDEX]} {int(info[WEDDING_YEAR_INDEX]) - int(people_dict[wife][BIRTH_YEAR_INDEX])}')
