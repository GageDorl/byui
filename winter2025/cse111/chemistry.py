...
def main():
    ...
    parsed_formula = parse_formula(formula, periodic_table_list)
    sample_mass = float(input('Enter a sample mass: '))
    print(f'{get_formula_name(formula, known_molecules_dict)}')
    print(f'{sum_protons(parsed_formula, periodic_table_list)} protons')
    molar_mass = compute_molar_mass(parsed_formula, periodic_table_list)
    print(f'{molar_mass:.5f} grams/mole\n{sample_mass / molar_mass:.5f} moles')
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        molar_mass += periodic_table_dict[element[SYMBOL_INDEX]][ATOMIC_MASS_INDEX] * element[QUANTITY_INDEX]
...
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    protons = 0
    for element in symbol_quantity_list:
        protons += periodic_table_dict[element[SYMBOL_INDEX]][ATOMIC_NUMBER_INDEX] * element[QUANTITY_INDEX]
    return protons
